...
public abstract class AbstractResourceHandler implements HttpHandler {
	static {
		// This is not an exhaustive list, just the most common types. Call
		// registerMimeType() to add more.
		...
		mimeTypes.put("js", "text/javascript"); // Technically it should be
	...
	private static Logger logger = Logger
			.getLogger(AbstractResourceHandler.class);
	...
	public AbstractResourceHandler enableDirectoryListing(
			boolean isDirectoryListingEnabled) {
		return enableDirectoryListing(isDirectoryListingEnabled,
				new DefaultDirectoryListingFormatter());
	}
	...
	public void handleHttpRequest(final HttpRequest request,
			final HttpResponse response, final HttpControl control)
			throws Exception {
		// Switch from web thead to IO thread, so we don't block web server when
		// we access the filesystem.
		ioThread.execute(createIOWorker(request, response, control));
	}

	protected void serve(final String mimeType, final byte[] staticContents,
			HttpControl control, final HttpResponse response,
			final HttpRequest request, final String path) {
		...
		control.execute(new Runnable() {
			...
			public void run() {
				...
				logger.info("## Tratando mimeType = " + mimeType);
				byte[] dynamicContents = templateEngine.process(staticContents,
						path, request.data(TemplateEngine.TEMPLATE_CONTEXT));
	...
	private boolean maybeServeRange(HttpRequest request, ByteBuffer contents,
		...
		if (null != startString && null != endString) {
			...
			if (start <= end) {
				serveRange(start, Math.min(contents.remaining() - 1, end),
						contents, response);
				return true;
			}
		} else if (null != startString) {
			serveRange(Integer.parseInt(startString), contents.remaining() - 1,
					contents, response);
			return true;
		} else if (null != endString) {
			int end = Integer.parseInt(endString);
			serveRange(contents.remaining() - end, contents.remaining() - 1,
					contents, response);
			return true;
		}
	...
	protected void serveRange(int start, int end, ByteBuffer contents,
			HttpResponse response) {
		if (start > contents.remaining()) {
			response.status(416)
					.header("Content-Range", "bytes */" + contents.remaining())
					.end();
			return;
		}
		...
		contents.limit(contents.position() + end + 1).position(
	...
	/**
	 * All IO is performed by this worker on a separate thread, so we never
	 * block the HttpHandler.
	 */
	protected abstract class IOWorker implements Runnable {
		...
		protected IOWorker(String path, HttpRequest request,
				HttpResponse response, HttpControl control) {
			this.path = path;
		...
		public void run() {
			...
			try {
				...
				if (isDirectory()) {
					// Assumes if path has been changed since the original
					// request,
					// its current value with a trailing slash will still
					// resolve properly
					if (!path.endsWith("/")) {
						response.status(301)
								.header("Location",
										path + "/"
												+ extractQuery(pathWithQuery))
								.end();
						return;
					} else if ((content = welcomeBytes()) != null) {
						serve(guessMimeType(welcomeFileName), content, control,
								response, request, path);
						return;
					} else if (isDirectoryListingEnabled
							&& (content = directoryListingBytes()) != null) {
						serve(guessMimeType(".html"), content, control,
								response, request, path);
						return;
					}
					// TODO: Do something other than 404 if directory listing is
					// disabled
		...
		// TODO: Don't respond with a mime type that violates the request's
		// Accept header
