...
public class StaticFileHandler extends AbstractResourceHandler {
	...
	public StaticFileHandler(File dir, Executor ioThread,
			TemplateEngine templateEngine, long maxAge) {
		super(ioThread, templateEngine);
		this.dir = dir;
		this.maxAge = maxAge;
	}
	...
	protected class FileWorker extends IOWorker {
		...
		private String mimeType(String uri) {
			String ext = uri.lastIndexOf(".") != -1 ? uri.substring(uri
					.lastIndexOf(".")) : null;
		...
		protected FileWorker(HttpRequest request, HttpResponse response,
				HttpControl control, long maxAge) {
		...
		protected byte[] fileBytes() throws IOException {
			...
			if (raw != null) {
				...
				if (request.header("If-Modified-Since") != null) {
					if (fromHeader(request.header("If-Modified-Since"))
							.getTime() >= lastModified.getTime()) {
						response.status(304);
					}
				}
				...
				if (maxAge != 0) {
					response.header("Expires",
							toHeader(new Date(new Date().getTime() + maxAge
									* 1000)));
		...
		protected byte[] directoryListingBytes() throws IOException {
			...
			Iterable<FileEntry> files = ClassloaderResourceHelper
					.fileEntriesFor(file.listFiles());
