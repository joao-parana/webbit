...
public class NettyWebServer implements WebServer {
	private static Logger logger = Logger.getLogger(NettyWebServer.class);
	...
	public NettyWebServer(int port) {
		this(Executors
				.newSingleThreadScheduledExecutor(new NamingThreadFactory(
						"WEBBIT-HANDLER-THREAD")), port);
	}

	private NettyWebServer(ExecutorService executorService, int port) {
		this((Executor) executorService, port);
		// If we created the executor, we have to be responsible for tearing it
		// down.
	...
	public NettyWebServer(final Executor executor, SocketAddress socketAddress,
			URI publicUri) {
		...
		// when reading/writing to the client. The Internet is flaky - it
		// happens.
	...
	@Override
	public NettyWebServer setupSsl(InputStream keyStore, String pass)
			throws WebbitException {
		return this.setupSsl(keyStore, pass, pass);
	}

	@Override
	public NettyWebServer setupSsl(InputStream keyStore, String storePass,
			String keyPass) throws WebbitException {
		this.sslContext = new SslFactory(keyStore, storePass)
				.getServerContext(keyPass);
		return this;
	}
	...
	public Future<NettyWebServer> start() {
		FutureTask<NettyWebServer> future = new FutureTask<NettyWebServer>(
				new Callable<NettyWebServer>() {
					@Override
					public NettyWebServer call() throws Exception {
						...
						bootstrap
								.setPipelineFactory(new ChannelPipelineFactory() {
									...
									public ChannelPipeline getPipeline()
										...
										pipeline.addLast(
												"staleconnectiontracker",
												staleConnectionTrackingHandler);
										pipeline.addLast("connectiontracker",
												connectionTrackingHandler);
										pipeline.addLast("flashpolicydecoder",
												new FlashPolicyFileDecoder(
														executor,
														exceptionHandler,
														ioExceptionHandler,
														getPort()));
						...
						staleConnectionTrackingHandler = new StaleConnectionTrackingHandler(
								staleConnectionTimeout, executor);
						ScheduledExecutorService staleCheckExecutor = Executors
								.newSingleThreadScheduledExecutor(new NamingThreadFactory(
										"WEBBIT-STALE-CONNECTION-CHECK-THREAD"));
						...
						ExecutorService workerExecutor = Executors
								.newSingleThreadExecutor(new NamingThreadFactory(
										"WEBBIT-WORKER-THREAD"));
						executorServices.add(workerExecutor);
						bootstrap.setFactory(new NioServerSocketChannelFactory(
								bossExecutor, workerExecutor, 1));
	...
	public Future<WebServer> stop() {
		FutureTask<WebServer> future = new FutureTask<WebServer>(
				new Callable<WebServer>() {
					@Override
	...
	@Override
	public NettyWebServer uncaughtExceptionHandler(
	...
	@Override
	public NettyWebServer connectionExceptionHandler(
